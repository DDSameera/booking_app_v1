...
<template>
    <AppLayout>
        ...
        <div class="filters">
            <div>Check-in: <b>{{ search.check_in }}</b></div>
            <div>Check-out: <b>{{ search.check_out }}</b></div>
            <div>Guests: <b>{{ search.guests }}</b></div>
        </div>
        ...
        <div class="pagination" v-if="rooms.links?.length">
            ...
            <a v-for="l in rooms.links" :key="l.label" :href="l.url || '#'" :class="{active: l.active, disabled: !l.url}" v-html="l.label"></a>
        </div>
    </AppLayout>
</template>
